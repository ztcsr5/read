...
class SourceDiagnosticViewModel extends StateNotifier<SourceDiagnosticState> {
  ...
  Future<void> loadHistory() async {
    final allHistory = await SourceDiagnosticHistory.getHistoryForSource(state.source.bookSourceUrl);
    final allHealth = await SourceHealthRecord.getRecordsForSource(state.source.bookSourceUrl);
    state = state.copyWith(history: allHistory, healthRecords: allHealth);
  }

  Future<void> runDiagnosis(String keyword) async {
    state = state.copyWith(isDiagnosing: true, error: null);
    try {
      final report = await SourceDiagnosticService.diagnose(state.source, keyword);
  ...
  Future<void> applyAutoRepair() async {
    state = state.copyWith(isDiagnosing: true, error: null);
    try {
      final repairedSource = SourceAutoRepairService.repair(state.source);
  ...
  Future<void> applyReverseChapters() async {
    state = state.copyWith(isDiagnosing: true, error: null);
    try {
      final repairedSource = SourceAutoRepairService.reverseChapters(state.source);
      await _repository.saveBookSource(repairedSource);
      state = state.copyWith(source: repairedSource);
      await runDiagnosis('斗破苍穹');
    } catch (e) {
  ...
  Future<void> clearHistory() async {
    await SourceDiagnosticHistory.clearHistoryForSource(state.source.bookSourceUrl);
    await SourceHealthRecord.clearHistoryForSource(state.source.bookSourceUrl);
    await loadHistory();
  }
}

final sourceDiagnosticViewModelProvider = StateNotifierProvider.family<SourceDiagnosticViewModel, SourceDiagnosticState, BookSource>((ref, source) {
