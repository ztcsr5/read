...
final class RSSArticleContentCacheStore: ObservableObject {
    ...
    func save(_ paragraphs: [String], for article: RSSArticlePreview) {
        let normalized = paragraphs
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        guard !normalized.isEmpty else { return }
        let entry = RSSArticleContentCacheEntry(articleID: article.id, paragraphs: normalized, cachedAt: Date())
